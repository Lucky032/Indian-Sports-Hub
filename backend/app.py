...
def view_coach_profile():
    ...
    try:
        ...
        cursor.execute('SELECT CoachProfile.* FROM CoachProfile INNER JOIN PlayerProfile ON CoachProfile.Game = PlayerProfile.Game')
        coach_profiles = cursor.fetchall()
        print(f"Fetched Coach Profiles: {coach_profiles}")

        # Get profile index from query parameter, defaulting to 0
        profile_index = int(request.args.get('index', 0))
        print(f"Profile Index: {profile_index}")  # Debugging line
        print(f"Total Profiles: {len(coach_profiles)}")
...
def view_player_profile():
    ...
    try:
        ...
        cursor.execute('SELECT PlayerProfile.* FROM PlayerProfile INNER JOIN CoachProfile ON PlayerProfile.Game = CoachProfile.Game')
